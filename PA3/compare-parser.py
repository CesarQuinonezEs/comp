...
def parse(parser_name, arg_str):
	cmd = "python2 %s %s" % (parser_name, arg_str)
...
def main(arg_str):
	...
	out_ref = parse("parser.py", arg_str).splitlines()

	out_my = parse("myparser.py", arg_str).splitlines()
	if  (len(out_my) > 0 and
			out_my[len(out_my)-1] == "Compilation halted due to lex and parse errors"):
		del out_my[-1]
